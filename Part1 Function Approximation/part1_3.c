...
int main(int argc, char *argv[])
{
    ...
    printf("z = x*x + y*y (x range [0, 1], y range [0, 1]).\n");
    ...
    for (int i = 0; i < num_Train; i++)
    {
        ...
        output_Train[i] = pow(input_Train[i][0], 2) + pow(input_Train[i][1], 2);
    ...
    {
        squareErrSum += pow(pow(input_Test[j][0], 2) + pow(input_Test[j][1], 2) - *genann_run(ann, input_Test[j]), 2);
        // printf("Output for x = %f, y = %f is %f. True value is %f. Error is %f.\n",
        //        input_Test[j][0], input_Test[j][1], *genann_run(ann, input_Test[j]),
        //        pow(input_Test[j][0], 2) + pow(input_Test[j][1], 2),
        //        pow(input_Test[j][0], 2) + pow(input_Test[j][1], 2) - *genann_run(ann, input_Test[j]));
    }
